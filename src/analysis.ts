...
class ReimbursementAnalyzer {
  ...
  public analyzeByDuration(): DurationAnalysis[] {
    ...
    return Array.from(byDays.entries())
      .map(([days, cases]) => ({
        days,
        caseCount: cases.length,
        averageOutput: this.calculateMean(cases.map(c => c.expected_output)),
        averagePerDay:
          this.calculateMean(cases.map(c => c.expected_output)) / days,
  ...
  public analyzeReceiptImpact(): void {
    ...
    console.log(
      'Looking for cases with similar trip parameters but different receipts...',
    );
      ...
      if (cases.length >= 3) {
        ...
        if (days <= 5) {
          ...
          cases
            .sort(
              (a, b) =>
                a.input.total_receipts_amount - b.input.total_receipts_amount,
            )
            .forEach(case_ => {
              const {miles_traveled, total_receipts_amount} = case_.input;
              console.log(
  ...
  public analyzeFiveDayTripsDetailed(): void {
    ...
    const fiveDayTrips = this.cases
      .filter(case_ => case_.input.trip_duration_days === 5)
      .sort((a, b) => a.input.miles_traveled / 5 - b.input.miles_traveled / 5);
    ...
    this.analyzeByDuration().forEach(analysis => {
      ...
      if (analysis.examples.length <= 5) {
        analysis.examples.forEach(case_ => {
          const {trip_duration_days, miles_traveled, total_receipts_amount} =
            case_.input;
          console.log(
